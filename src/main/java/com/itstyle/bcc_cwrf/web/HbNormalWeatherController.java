...
//        平均气温对应的是AT2M，日最高气温对应的是T2MAX。日最低气温对应的是T2MIN，降水就是PRAVG，风速就是WSAVG
    ...
    public Result climatePicture(@RequestBody Map<String, Object> map){
        log.info("获取的数据：{}", map);
        String resultPath = hbNormalWeatherService.weatherSecondPicture(map);
        if(StringUtils.isNotBlank(resultPath)){
            return Result.ok(resultPath);
        }else{
            return Result.error(errorMsg);
    ...
    public Result anomalyPicture(@RequestBody Map<String, Object> map){
        log.info("获取的数据：{}", map);
        Result resultPath = hbNormalWeatherService.weatherThirdPicture(map);
        return resultPath;
    }

    @ApiOperation(value = "气候回报数据 常规气候预测产品生成模块 执行距平格点出图")
    @PostMapping("/execAnomaly")
    public Result execAnomalyPicture(@RequestBody Map<String, Object> map){
        log.info("获取的数据：{}", map);
        List<String> resultPath = hbNormalWeatherService.execWeatherThirdPicture(map);
        return Result.ok(resultPath);
    }

    @ApiOperation(value = "气候回报数据 常规气候预测产品生成模块 执行距平格点出图")
    @PostMapping("/batchExecAnomaly")
    public Result batchExecAnomalyPicture(@RequestBody Map<String, Object> map){
        log.info("获取的数据：{}", map);
        List<Map<String, Object>> resultPath = hbNormalWeatherService.batchExecWeatherThirdPicture(map);
        return Result.ok(resultPath);
    }
